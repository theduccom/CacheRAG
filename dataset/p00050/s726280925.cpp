...
//	freopen("testcase.applepeach", "r", stdin);
	...
	while (getline (cin, str ) ){
		...
		while (str.find(w[0] ) != string::npos || str.find(w[1]) != string::npos  ){
			if (str.find (w[0] ) != string::npos){
				str = str.replace (str.find (w[0]), w[0].length(),t[0] );
			} // end if
			if (str.find (w[1] ) != string::npos){
				str = str.replace (str.find (w[1]), w[1].length(),t[1] );
			} // end if
		} // end while

		while (str.find (t[0] ) != string::npos || str.find (t[1]) != string::npos  ){
			if (str.find (t[0] ) != string::npos){
				str = str.replace (str.find (t[0]), t[0].length(),w[1] );
			} // end if
			if (str.find (t[1] ) != string::npos){
